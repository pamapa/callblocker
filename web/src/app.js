...
require([
    ...
    function date2UTCString(date)
    {
        ...
        var str = date.getUTCFullYear() + "-" + pad(date.getUTCMonth() + 1) + "-" + pad(date.getUTCDate()) + " " + pad(date.getUTCHours()) + ":" + pad(date.getUTCMinutes()) + ":" + pad(date.getUTCSeconds()) + " +0000";
        return str;
    }
            ...
            if (items.length) {
                ...
                dojo.forEach(items, function(si) {
                    if (si !== null) {
                        ...
                        var newItem = { id : uuid4(), number, name, date_created : dateStr, date_modified : dateStr };
    ...
    function createPhone()
    {
        ...
        var structure = [
            { name : "Enabled", field : "enabled", width : "40px", type : dojox.grid.cells._Widget, formatter : function(on) {
            ...
            { name : "Block anonymous CID", field : "block_anonymous_cid", width : "60px", type : dojox.grid.cells._Widget, formatter : function(on) {
                    return new dijit.form.CheckBox({ checked : on, readOnly : true });
                } },
            { name : "Block invalid CID", field : "block_invalid_cid", width : "60px", type : dojox.grid.cells._Widget, formatter : function(on) {
                    return new dijit.form.CheckBox({ checked : on, readOnly : true });
                } },
            ...
            { name : "SIP password", field : "password", width : "160px", type : dojox.grid.cells._Widget, formatter : function(str) {
            ...
            onClick : function() {
                ...
                if (items.length) {
                    ...
                    var myDialog = new ConfirmDialog({
                        title : "Edit entry",
                        content : [
                            domConstruct.create("T", { innerHTML : "Type: " }),
                            typeSelect.domNode,
                            domConstruct.create("br"),
                            domConstruct.create("T", { innerHTML : "Enabled: " }),
                            enabledCheckBox.domNode,
                            domConstruct.create("br"),
                            domConstruct.create("T", { innerHTML : "Name: " }),
                            nameTextBox.domNode,
                            domConstruct.create("br"),
                            domConstruct.create("T", { innerHTML : "Country code: " }),
                            countryCodeTextBox.domNode,
                            domConstruct.create("br"),
                            domConstruct.create("T", { innerHTML : "Block mode: " }),
                            blockModeSelect.domNode,
                            domConstruct.create("br"),
                            domConstruct.create("T", { innerHTML : "Block anonymous CID: " }),
                            blockAnonymousCIDCheckBox.domNode,
                            domConstruct.create("br"),
                            domConstruct.create("T", { innerHTML : "Block invalid CID: " }),
                            blockInvalidCIDCheckBox.domNode,
                            domConstruct.create("br"),
                            domConstruct.create("T", { innerHTML : "Online check: " }),
                            onlineCheckSelect.domNode,
                            domConstruct.create("br"),
                            domConstruct.create("T", { innerHTML : "Online lookup: " }),
                            onlineLookupSelect.domNode,
                            domConstruct.create("br"),
                            domConstruct.create("T", { innerHTML : "Device: " }),
                            deviceTextBox.domNode,
                            domConstruct.create("br"),
                            domConstruct.create("T", { innerHTML : "SIP domain: " }),
                            domainTextBox.domNode,
                            domConstruct.create("br"),
                            domConstruct.create("T", { innerHTML : "SIP username: " }),
                            usernameTextBox.domNode,
                            domConstruct.create("br"),
                            domConstruct.create("T", { innerHTML : "SIP password: " }),
                            passwordTextBox.domNode,
                            domConstruct.create("br"),
                        ],
                        onExecute : function() {
    ...
    function createOnlineCredentials(url)
    {
        ...
        var structure = [
            ...
            { name : "Password", field : "password", width : "200px", type : dojox.grid.cells._Widget, formatter : function(str) {
                    return new dijit.form.TextBox({ value : str, type : "password", readOnly : true });
                } }
        ];
        var grid = new dojox.grid.EnhancedGrid({
        ...
        var editMenuItem = new dijit.MenuItem({
            ...
            onClick : function() {
                ...
                if (items.length) {
                    ...
                    var myDialog = new ConfirmDialog({
                        title : withNumber ? "Edit number or name" : "Edit name (? matches any single character and * matches any string including empty string, e.g. SPAM*)",
    ...
    function createTree()
    {
        var treeData = {
            ...
            label : "name",
            items : [
                { id : "root", name : "Root", func : null, children : [ { _reference : "calllog" }, { _reference : "config" }, { _reference : "diag" } ] },
                { id : "calllog", name : "Caller Log", func : createCallerLogGrid },

                // Configuration
                { id : "config", name : "Configuration", func : null, children : [
                        { _reference : "config_phone" }, { _reference : "config_onlinecreds" },
                        { _reference : "config_allowlists" }, { _reference : "config_blocklists" }
                    ] },
                { id : "config_phone", name : "Phone", func : createPhone }, { id : "config_onlinecreds", name : "Online Credentials", func : createOnlineCredentials }, { id : "config_allowlists", name : "Allowlists", func : createAllowlist }, { id : "config_blocklists", name : "Blocklists", func : createBlocklist },

                // Diagnostic
                { id : "diag", name : "Diagnostic", func : null, children : [ { _reference : "diag_cache" }, { _reference : "diag_logs" } ] }, { id : "diag_cache", name : "Cache", func : null, children : [
                                                                                                                                                    { _reference : "diag_cache_onlinelookup" }, { _reference : "diag_cache_onlinecheck" }
                                                                                                                                                ] },
                { id : "diag_cache_onlinelookup", name : "Online Lookup", func : createOnlineLookuplist }, { id : "diag_cache_onlinecheck", name : "Online Check", func : createOnlineChecklist }, { id : "diag_logs", name : "Logs", func : null, children : [
                                                                                                                                                                                                        { _reference : "diag_logs_error_warn" }, { _reference : "diag_logs_all" }
                                                                                                                                                                                                    ] },
                { id : "diag_logs_error_warn", name : "Error and Warning", func : createJournalErrorWarnGrid }, { id : "diag_logs_all", name : "All", func : createJournalAllGrid }
            ]
        };
